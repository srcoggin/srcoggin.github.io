...
export default function DeepDive({ data }: DeepDiveProps) {
  ...
  // A player is a true rookie only if their first season in our data matches selectedYear
  // AND we have data from the year before (so we can confirm they weren't playing earlier)
  const canVerifyRookies = data.some(d => d.season === selectedYear - 1)
  const isRookie = canVerifyRookies && selectedPlayer && selectedYear === playerFirstSeason[selectedPlayer.name]
  ...
  return (
    <div className="min-w-0 w-full">
      <h2 className={`text-xl sm:text-2xl font-bold mb-4 sm:mb-6 break-words ${textPrimary} flex items-center gap-2`}>
            ...
            {uniquePlayers.length === 0 ? (
              ...
              <Autocomplete
                options={uniquePlayers.map(p => {
                  const rookieInYear = canVerifyRookies && selectedYear === playerFirstSeason[p.name]
            ...
            <>
              ...
              {selectedPlayer.position !== 'K' && (
                <>
                  ...
                  <Expander title={<span className="flex items-center gap-2"><IconBook size={16} /> Metric Key (Defence ranks)</span>} defaultExpanded={false} className="mb-3">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="font-medium mb-1 text-[var(--accent-green)] flex items-center gap-1.5">
                          <IconCircleGreen size={14} /> Green Highlights
                        </p>
                        <p className={`text-xs ${textSecondary}`}>• Top 10 defence (rank 1–10)</p>
                      </div>
                      <div>
                        <p className="font-medium mb-1 text-[var(--accent-red)] flex items-center gap-1.5">
                          <IconCircleRed size={14} /> Red Highlights
                        </p>
              ...
              {/* Box Score Legend */}
              <Expander title={<span className="flex items-center gap-2"><IconBook size={16} /> Box Score Legend</span>} defaultExpanded={false} className="mb-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="font-medium mb-1 text-[var(--accent-green)] flex items-center gap-1.5">
                      <IconCircleGreen size={14} /> Green Highlights
